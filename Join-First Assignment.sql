...
select  FirstName , LastName, Department.DepartmentName
from employee 
...
SELECT Employee.FirstName, Employee.LastName, Department.Location
FROM Employee
INNER JOIN Department ON Employee.DepartmentID = Department.DepartmentID;

-- 4. Retrieve the total number of employees in each department
Select Department.DepartmentName, count(EmployeeID) from Employee
inner join Department
on employee.DepartmentID = Department.DepartmentID
group by Department.DepartmentName;
 
-- 5. Find the highest-paid employee in each department
Select Department.DepartmentName, Max(Salary) from Employee
join Department
on employee.DepartmentID = Department.DepartmentID
group by Department.DepartmentName;

-- 6. Display employees who joined after January 1, 2020, along with their department names
Select FirstName, LastName, DateOfJoining, Department.DepartmentName from Employee
inner join Department
on employee.DepartmentID = Department.DepartmentID
where DateOfJoining > '2020-01-10';

-- 7. Show employees earning more than ₹70,000 and their respective departments
Select EmployeeID, FirstName, LastName, Email, Salary, DateOfJoining, Department.DepartmentName from Employee
inner join Department
on employee.DepartmentID = Department.DepartmentID
where Salary > 70000;

-- 8. Retrieve employees working in the "Pune" office
SELECT EmployeeID, FirstName, LastName, Email, Salary, DateOfJoining, Department.DepartmentName, Department.Location
FROM Employee
INNER JOIN Department 
on Employee.DepartmentID = Department.DepartmentID
Where Department.Location = 'Pune';
...
Select Department.DepartmentName, avg(Salary) from Employee
inner join Department
on employee.DepartmentID = Department.DepartmentID
group by Department.DepartmentName;
...
LEFT JOIN Employee e ON e.DepartmentID = d.DepartmentID
GROUP BY d.DepartmentName
having COUNT(e.EmployeeID) = (SELECT COUNT(e.EmployeeID) AS TotalEmployees
FROM Department d
LEFT JOIN Employee e ON e.DepartmentID = d.DepartmentID
GROUP BY d.DepartmentName
ORDER BY COUNT(e.EmployeeID) DESC
...
Select EmployeeID, FirstName, LastName, Employee.DepartmentID from employee
...
Select EmployeeID, FirstName, LastName, Salary, Department.DepartmentName from employee
Inner Join Department
on Employee.DepartmentID = Department.DepartmentID
...
SELECT EmployeeID, FirstName, LastName, Email, Salary, DateOfJoining, Department.DepartmentName
FROM Employee 
JOIN Department ON Employee.DepartmentID = Department.DepartmentID
WHERE DepartmentName = (SELECT DepartmentName
    FROM Department
    Join Employee
    on Department.DepartmentID = Employee.DepartmentID  
    WHERE FirstName = 'Amit' AND LastName = 'Sharma');
...
Select Department.DepartmentID, DepartmentName, Location, EmployeeID, FirstName, LastName from Department
Left Join Employee 
on Department.DepartmentID = Employee.DepartmentID;
...
SELECT Department.DepartmentID, DepartmentName, Location FROM Department 
LEFT JOIN Employee 
ON Department.DepartmentID = Employee.DepartmentID
...
SELECT DepartmentName, Location, EmployeeID, FirstName, LastName, Salary
FROM Department 
