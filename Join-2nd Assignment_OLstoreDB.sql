...
Select OrderID, O.CustomerID, O.ProductID, OrderDate, Quantity, TotalAmount, PaymentStatus, C.FirstName, C.LastName from Orders as O
join Customers as C
Join Products as P
on O.OrderID = C.CustomerID and O.OrderID = P.ProductID;

-- 2. Display the product names and their respective categories.
Select ProductID, ProductName, Category from Products as P;


-- 3. List the order details, including product names, quantities, and prices.
Select OrderID, O.CustomerID, O.ProductID, OrderDate, Quantity, TotalAmount, PaymentStatus, P.ProductName, P.price from Orders as O
-- join Customers as C
Join Products as P
