...
Select avg(DeliveryDate) from Orders;
...
Select min(paymentmethod) from orders;

Select OrderID, CustomerName, PaymentMethod from Orders
Where PaymentMethod in (select PaymentMethod from Orders
group by PaymentMethod
Having count( distinct customerName) > 1);

select PaymentMethod from Orders
group by PaymentMethod
Having count(distinct customerName) > 1;

select * from Orders;
-- 7. Find customers who ordered products priced above the average for their city.
Select avg(city) from Orders;
