...
select department, avg(grade) as Avg_Grade from students 
group by department;

-- Question 2: List department with more than 2 students.
select department, count(student_id) as Student_Count from students
group by department
having count(student_id) > 2;

-- Question 3: Find the highest grade in each department.
select department, sum(grade) as Highest_Grade from students
group by department
...
select avg(grade) as Avg_Grade, count(grade) as Count_Of_Grade from students
group by grade
