...
select product_name, count(sale_id) as Sale_Count, avg(sale_amount) as Average_Sale_Amount from sales
group by product_name
having count(sale_id)  >= 2;

update sales 
set product_name = "T-shirt"
where sale_id = 2;

-- 8-	Which products have the highest sale amount above 100?

select product_name, sum(sale_amount) as Sale_Amount from sales
group by product_name
having sum(sale_amount) > 100;


-- 9- Which product has the highest total sales amount?
select product_name, sum(sale_amount) as Highest_Sale_Amount from sales
group by product_name
order by Highest_Sale_Amount desc
limit 1;

-- 10-	Which categories have a total sales amount above 800?
select category, sum(sale_amount) as Total_Sale_Amount from sales
group by category
having sum(sale_amount) > 800;

-- 11-	What is the average sale amount for each month?
select monthname(sale_date), avg(sale_amount) from sales 
...
from (
select category, product_name,sale_amount, rank() over (partition by category order by sale_amount desc) as rnk from sales
)t
where rnk = 1;


-- 13-	How many sales occurred for each month?
select monthname(sale_date), sum(sale_amount) as Total_Sale_Amount from sales
group by monthname(sale_date);


-- 14-	In which months are there more than 5 sales?
select monthname(sale_date)as Month_Name, count(sale_date) as Sale_Count from sales
group by monthname(sale_date)
having count(month(sale_date))>5;

-- 15-	What is the average sale amount for each product category in months with more than 3 sales?
select avg(sale_amount), monthname(sale_date), count(sale_id) from sales 
group by monthname(sale_date)
