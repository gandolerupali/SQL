...
-- Find the brand with the most car models in the dataset.
...
-- List cars with prices higher than the average price of cars manufactured in the year 2021.
Select avg(price) from cars;         -- 59029.050000
...
-- Retrieve brands with more than two car models in the dataset.
select Brand, count(Model) from cars
group by Brand
...
-- List cars with prices higher than the average price of cars with the same transmission type and fuel type.
Select * from Cars
Where price > (Select avg(price) from Cars
where transmissiontype = 'Automatic' and fueltype = 'Gasoline');

-- Find the brand with the highest total mileage across all models.
