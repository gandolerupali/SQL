...
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(51, 'Mango', 5, 160.00, '2025-08-26', 'Shobha Minde', 'Pune');


-- 2.	Insert a sale of 12 kg Papaya in Mumbai at 55 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(52, 'Papaya', 12, 55.00, '2025-08-26', 'Nita Shekh', 'Mumbai');


-- 3.	Insert a sale of 10 kg Grapes in Nagpur at 90 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(53, 'Grapes', 10, 90.00, '2025-08-27', 'Nutan Hande', 'Nagpur');


-- 4.	Add a sale of 20 kg Oranges in Aurangabad at 65 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(54, 'Orange', 20, 65.00, '2025-08-27', 'Sumit Wale', 'Aurangabad');


-- 5.	Insert 15 kg of Watermelon sold in Pune at 30 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(55, 'Watermelon', 15, 30.00, '2025-08-28', 'Rani Kale', 'Pune');


-- 6.	Insert 8 kg Apples sold in Nashik at 125 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(56, 'Apple', 8, 125.00, '2025-08-28', 'Sangita Godse', 'Nashik');


-- 7.	Add 10 kg Bananas sold in Mumbai at 38 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(57, 'Banana', 10, 38.00, '2025-08-29', 'Sharvil Mali', 'Mumbai');


-- 8.	Insert a Mango sale of 25 kg in Nagpur at 155 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(58, 'Mango', 25, 155.00, '2025-09-01', 'Krishiv Gaje', 'Nagpur');


-- 9.	Insert 9 kg Papaya sold in Aurangabad at 60 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(59, 'Papaya', 9, 60.00, '2025-09-02', 'Raj Thakur', 'Aurangabad');


-- 10.	Insert 11 kg Oranges in Pune at 63 per kg.
insert into Fruit_Sales (sale_id, fruit_name, quantity, price_per_kg, sale_date, customer_name, city) VALUES
(60, 'Oranges', 11, 63.00, '2025-09-03', 'Viki Gupta', 'Pune');
...
select fruit_name, sum(quantity) as Total_of_Fruits from Fruit_Sales
group by fruit_name;


-- 22.	Find the average price per kg for each fruit.
select fruit_name, avg(price_per_kg) as AvgPrice_of_Fruits from Fruit_Sales
group by fruit_name;


-- 23.	Find the maximum quantity sold for each fruit.
select fruit_name, max(quantity) as MaxQuantity_of_Fruits from Fruit_Sales
group by fruit_name;


-- 24.	Count how many sales records exist for each city.
select city, count(sale_id) as Count_of_SaleRecord from Fruit_Sales
group by city;


-- 25.	Find the minimum price per kg for each fruit.
select fruit_name, min(price_per_kg) as MinPrice_of_Fruits from Fruit_Sales
group by fruit_name;


-- 26.	Show average quantity sold grouped by city.
select city, avg(quantity) as AvgQuantity_of_Fruits from Fruit_Sales
group by city;


-- 27.	Find total sales (quantity × price) grouped by fruit_name.
select fruit_name, sum(quantity) as TotalQuantity_of_Fruits, sum(price_per_kg) as TotalPrice_of_Fruits from Fruit_Sales
group by fruit_name;


-- 28.	Calculate total revenue for Mango sales.
select fruit_name, sum(price_per_kg) as Total_Revenue from Fruit_Sales
group by fruit_name
having fruit_name = 'Mango';



-- 29.	Show number of sales per day.
select sale_date, count(sale_id) as Total_Sale from Fruit_Sales
group by sale_date;


-- 30.	Find the highest and lowest price of Apples sold.;
select fruit_name, min(price_per_kg) as Lowest_Price, max(price_per_kg) as Highest_Price from Fruit_Sales
group by fruit_name
...
update Fruit_Sales
set customer_name = 'Updated_Customer'
where sale_id = 5;
